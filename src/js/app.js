...
let camera, scene, renderer, controlWs, box, geometry, material, controls
let colors = {}
let points;
let pointsCount = 0
...
function initGui() {
	...
	optionFolder.add(opOp, 'pointsCount').listen()
...
function init() {
	...
	material = new THREE.PointsMaterial({ 	// TODO: Change shape can be done with a texture but not cool maybe shader? not sure what any paras do
		size: 20,
		vertexColors: true,
